...
def newLandmark(json):
    try:
        req = requests.get(
            "http://localhost:5000/setLandmark/" + json["name"] + "/" + str(json["x"]) + "/" + str(json["y"]))
        if (req.status_code == 200):
            updateLocations()
    except:
        error("[ERROR] Failed Set Landmark")
...
def goTo(json):
    try:
        ...
        if (req.status_code == 200):
            updateLocations()
            success("[SUCCESS] Go Success")
            # console.log("on way")
    except:
        error("[ERROR] Failed Set Goal")
...
def find(json):
    try:
        ...
        if (req.status_code == 200):
            updateLocations()
            success("[SUCCESS] Find start Success")
            # console.log("on way")
    except:
        error("[ERROR] Failed to start Goal")
...
def say(json):
    try:
        ...
        error("[ERROR] Failed to Say from app.py")
...
def removeLandmark(json):
    try:
        ...
        error("[ERROR] Failed To remove landmark")
